...
fun containsIn(a: Map<String, String>, b: Map<String, String>): Boolean {
    a.forEach { if (!b.containsKey(it.key) || (b[it.key] != it.value)) return false }
    return true
}
...
fun bagPacking(treasures: Map<String, Pair<Int, Int>>, capacity: Int): Set<String> {
    ...
    var max: MutableSet<String> = mutableSetOf()
    treasures.forEach {
        if (it.value.first <= capacity) {
            memory[mutableSetOf(it.key)] = it.value
        }
    }
    var templates = memory.toMutableMap()
    for (g in 0 until treasures.size - 1) {
        ...
        templates.maxBy { it.value.second }?.key.apply {
            if (this != null)
                max = this
        }
